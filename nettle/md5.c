...
void md5_digest(struct md5_ctx *ctx, unsigned length, uint8_t *digest) {
  ...
  /* Little endian order */
  for (i = 0; i < words; i++, digest += 4) LE_WRITE_UINT32(digest, ctx->digest[i]);
  ...
  if (leftover) {
    ...
    for (word = ctx->digest[i], j = 0; j < leftover; j++, word >>= 8) digest[j] = word & 0xff;
  }
  md5_init(ctx);
}
...
static void md5_block(struct md5_ctx *ctx, const uint8_t *block) {
  ...
  /* Update block count */
  if (!++ctx->count_l) ++ctx->count_h;

  /* Endian independent conversion */
  for (i = 0; i < 16; i++, block += 4) data[i] = LE_READ_UINT32(block);
...
static void md5_final(struct md5_ctx *ctx) {
  ...
  words = i >> 2;
  for (i = 0; i < words; i++) data[i] = LE_READ_UINT32(ctx->block + 4 * i);

  if (words > (MD5_DATA_LENGTH - 2)) { /* No room for length in this block. Process it and
