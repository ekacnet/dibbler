...
void error_message_set(int errCode) {
  ...
  FormatMessage(
      ...
      NULL, errCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)tmp, ERROR_MESSAGE_SIZE - 10,
      NULL);
  sprintf(Message, "Error %d: %s\n", errCode, tmp);
}

void error_message_set_string(char* str) { strncpy(Message, str, ERROR_MESSAGE_SIZE); }

void if_list_release(struct iface* list) {
  struct iface* tmp;
  while (list) {
    tmp = list->next;
    if (list->linkaddrcount) free(list->linkaddr);
    free(list);
    list = tmp;
  }
}
...
extern struct iface* if_list_get() {
  ...
  while (adaptaddr) {
    ...
    WideCharToMultiByte(CP_ACP, 0, adaptaddr->FriendlyName, -1, iface->name, MAX_IFNAME_LENGTH,
                        NULL, NULL);
    ...
    if (adaptaddr->IfType == IF_TYPE_SOFTWARE_LOOPBACK) iface->flags |= IFF_LOOPBACK;
...
extern int is_addr_tentative(char* ifacename, int iface, char* plainAddr) {
  ...
  while (adaptaddr && (!found)) {
    ...
    while (linkaddr && (!found)) {
      addrpck = (struct sockaddr_in6*)linkaddr->Address.lpSockaddr;
      if (!memcmp((struct in6_addr*)(&addrpck->sin6_addr), netAddr, 16)) found = linkaddr;
      linkaddr = linkaddr->Next;
    }
    adaptaddr = adaptaddr->Next;
  }
...
extern int ipaddr_add(const char* ifacename, int ifaceid, const char* addr, unsigned long pref,
                      unsigned long valid, int prefixLen) {
  // netsh interface ipv6 add address interface="eth0" address=2000::123 validlifetime=120
  ...
  if (i) {
    i = _spawnl(_P_WAIT, netshPath, netshPath, arg1, arg2, "set", arg4, arg5, arg6, arg7, arg8,
                NULL);
  }
  return i;
}

int ipaddr_update(const char* ifacename, int ifindex, const char* addr, unsigned long pref,
...
extern int sock_add(char* ifacename, int ifaceid, char* addr, int port, int thisifaceonly,
  ...
  if ((s = socket(AF_INET6, SOCK_DGRAM, 0)) == INVALID_SOCKET) return -2;

  memset(&bindme, 0, sizeof(bindme));
  bindme.sin6_family = AF_INET6;
  bindme.sin6_port = htons(port);

  if (IN6_IS_ADDR_LINKLOCAL((IN6_ADDR*)packedAddr)) bindme.sin6_scope_id = ifaceid;
...
int sock_del(int fd) { return closesocket(fd); }
int sock_send(int fd, char* addr, char* buf, int buflen, int port, int iface) {
  ...
  if (getaddrinfo(addrStr, portStr, &inforemote, &remote)) return 0;
...
extern int dns_add(const char* ifname, int ifaceid, const char* addrPlain) {
  ...
  if (i == 0) {
    ...
    sprintf(Message, "%s %s %s %s %s %s %s returned non-zero returncode %d", netshPath, arg1, arg2,
            arg3, arg4, arg5, arg6, i);
    return LOWLEVEL_ERROR_UNSPEC;
  }
}
...
int prefix_add(const char* ifname, int ifindex, const char* prefixPlain, int prefixLength,
  ...
  // netsh interface ipv6 add route 2000::/64 "eth1" preferredlifetime=1000 validlifetime=2000
  // store=active [publish=age]
  char arg1[] = "interface";
  char arg2[] = "ipv6";
  char arg3[] = "add";
  char arg4[] = "route";
  ...
  sprintf(buf, "%s %s %s %s %s %s %s %s %s %s", arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8,
          arg9, arg10);
  i = _spawnl(_P_WAIT, netshPath, netshPath, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9,
              arg10, NULL);
...
int get_mac_from_ipv6(const char* iface_name, int ifindex, const char* v6addr, char* mac,
                      int* mac_len) {
