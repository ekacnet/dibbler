...
int lowlevelInit() { return LOWLEVEL_NO_ERROR; }
...
void if_list_release(struct iface *list) {
  ...
  while (list) {
    ...
    if (list->globaladdrcount) free(list->globaladdr);
    free(list);
    list = tmp;
  }
}
...
struct iface *if_list_get() {
  ...
  while (addr_ptr != NULL) {
    ...
    if (addr_ptr->ifa_addr->sa_family == AF_INET6) {
      ...
      printf(
          "Link-local addr: %02x%02x:%02x%02x:%02x%02x:%02x%02x"
    ...
    while (iface_ptr != NULL) {
      if (!strcmp(addr_ptr->ifa_name, iface_ptr->name)) break;
  ...
  for (addr_ptr = addrs_lst; addr_ptr != NULL; addr_ptr = addr_ptr->ifa_next) {
    for (iface_ptr = iface_lst; iface_ptr != NULL; iface_ptr = iface_ptr->next) {
      ...
      if (strncmp(iface_ptr->name, addr_ptr->ifa_name, strlen(addr_ptr->ifa_name))) continue;
...
#endif
  status = system(buf);
  if (!status) return LOWLEVEL_NO_ERROR;
...
int sock_add(char *ifacename, int ifaceid, char *addr, int port, int thisifaceonly, int reuse) {
  ...
  printf("LOWLEVEL_DEBUG iface: %s(id=%d), addr=%s, port=%d, ifaceonly=%d reuse=%d###\n", ifacename,
         ifaceid, addr, port, thisifaceonly, reuse);
  fflush(stdout);
#endif
...
#error "Both IPV6_RECVPKTINFO and IPV6_PKTINFO not defined. Need at least one of them"
  ...
  if (setsockopt(Insock, IPPROTO_IPV6, IPV6_RECVPKTINFO, &on, sizeof(on)) < 0)
    ...
    {
      sprintf(Message,
              "Failed to set up socket option (tried both IPV6_RECVPKTINFO and IPV6_PKTINFO).");
...
int sock_del(int fd) { return close(fd); }
...
int sock_recv(int fd, char *myPlainAddr, char *peerPlainAddr, char *buf, int buflen) {
  ...
  for (cm = (struct cmsghdr *)CMSG_FIRSTHDR(&msg); cm;
    ...
    if (cm->cmsg_level != IPPROTO_IPV6 || cm->cmsg_type != IPV6_PKTINFO) continue;
 ...
 *
 */
void link_state_change_cleanup() { return; }
