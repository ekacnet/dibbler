...
FILE* resolvconf_open(const char* arg1, const char* arg2) {
  ...
  if (access(RESOLVCONF, X_OK) != 0) return NULL;
 ...
 */
int cfg_value_del(FILE* in, FILE* out, const char* keyword, const char* value, const char comment,
                  int remove_empty) {
...
int cfg_file_del(const char* file, const char* keyword, const char* value) {
  ...
  if ((tmpfd = mkstemp(template)) == -1) return (LOWLEVEL_ERROR_FILE);
...
int domain_add(const char* ifname, int ifaceid, const char* domain) {
  ...
  if (!(f = fopen(RESOLVCONF_FILE ".old", "r"))) return LOWLEVEL_ERROR_FILE;
