  ...
  for (rta1 = (struct rtattr *)rtadata; RTA_OK(rta1, rtasize1); rta1 = RTA_NEXT(rta1, rtasize1)) {
    ...
    switch (rta1->rta_type) {
      case IFLA_INET6_FLAGS:
        ...
        if (mo_flags & IF_RA_MANAGED) printf("M flag!");
        if (mo_flags & IF_RA_OTHERCONF) printf("O flag!");
#endif
...
int ipaddr_add_or_del(const char *addr, const char *ifacename, int prefixLen,
  ...
  if (req.ifa.ifa_prefixlen == 0) req.ifa.ifa_prefixlen = lcl.bitlen;

  if (!scoped) req.ifa.ifa_scope = default_scope(&lcl);
