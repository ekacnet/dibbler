...
void rtnl_close(struct rtnl_handle *rth) { close(rth->fd); }
...
int rtnl_dump_filter(struct rtnl_handle *rth, rtnl_filter_t filter, void *arg1, rtnl_filter_t junk,
                     void *arg2) {
  struct sockaddr_nl nladdr;
  struct iovec iov;
  struct msghdr msg = {
...
int rtnl_talk(struct rtnl_handle *rtnl, struct nlmsghdr *n, pid_t peer, unsigned groups,
              struct nlmsghdr *answer, rtnl_filter_t junk, void *jarg) {
  int status;
  unsigned seq;
  struct nlmsghdr *h;
  struct sockaddr_nl nladdr;
    ...
    for (h = (struct nlmsghdr *)buf; status >= sizeof(*h);) {
      ...
      if (h->nlmsg_type == NLMSG_ERROR) {
        ...
        if (l < sizeof(struct nlmsgerr)) {
          ...
          errno = -err->error;
          if (errno == 0) {
            if (answer) memcpy(answer, h, h->nlmsg_len);
            return 0;
          }
          perror("RTNETLINK answers");
        }
...
int parse_rtattr_byindex(struct rtattr *tb[], int max, struct rtattr *rta, int len) {
  ...
  while (RTA_OK(rta, len)) {
    if (rta->rta_type <= max && i < max) tb[i++] = rta;
    rta = RTA_NEXT(rta, len);
  }
  if (len) fprintf(stderr, "!!!Deficit %d, rta_len=%d\n", len, rta->rta_len);
  return i;
