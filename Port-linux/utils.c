...
int get_integer(int *val, const char *arg, int base) {
  ...
  if (!ptr || ptr == arg || *ptr || res > INT_MAX || res < INT_MIN) return -1;
  *val = res;
  return 0;
}
...
int get_unsigned(unsigned *val, const char *arg, int base) {
  ...
  if (!ptr || ptr == arg || *ptr || res > UINT_MAX) return -1;
  *val = res;
  return 0;
}
...
int get_u32(__u32 *val, const char *arg, int base) {
  ...
  if (!arg || !*arg) return -1;
  res = strtoul(arg, &ptr, base);
  if (!ptr || ptr == arg || *ptr || res > 0xFFFFFFFFUL) return -1;
  *val = res;
  return 0;
}
...
int get_u16(__u16 *val, const char *arg, int base) {
  ...
  if (!arg || !*arg) return -1;
  res = strtoul(arg, &ptr, base);
  if (!ptr || ptr == arg || *ptr || res > 0xFFFF) return -1;
  *val = res;
  return 0;
}
...
int get_u8(__u8 *val, const char *arg, int base) {
  ...
  if (!arg || !*arg) return -1;
  res = strtoul(arg, &ptr, base);
  if (!ptr || ptr == arg || *ptr || res > 0xFF) return -1;
  *val = res;
  return 0;
}
...
int get_s16(__s16 *val, const char *arg, int base) {
  ...
  if (!arg || !*arg) return -1;
  res = strtol(arg, &ptr, base);
  if (!ptr || ptr == arg || *ptr || res > 0x7FFF || res < -0x8000) return -1;
  *val = res;
  return 0;
}
...
int get_s8(__s8 *val, const char *arg, int base) {
  ...
  if (!arg || !*arg) return -1;
  res = strtol(arg, &ptr, base);
  if (!ptr || ptr == arg || *ptr || res > 0x7F || res < -0x80) return -1;
  *val = res;
  return 0;
}
...
int get_addr_1(inet_prefix *addr, const char *name, int family) {
  ...
  if (strchr(name, ':')) {
    ...
    if (inet_pton(AF_INET6, name, addr->data) <= 0) return -1;
    addr->bytelen = 16;
    addr->bitlen = -1;
    return 0;
  }
  ...
  if (family == AF_DECnet) {
    ...
    if (dnet_pton(AF_DECnet, name, &dna) <= 0) return -1;
...
int matches(const char *cmd, const char *pattern) {
  int len = strlen(cmd);
  if (len > strlen(pattern)) return -1;
...
int inet_addr_match(const inet_prefix *a, const inet_prefix *b, int bits) {
  ...
  if (words)
    if (memcmp(a1, a2, words << 2)) return -1;
...
char *resolve_address(const char *addr, int len, int af) {
  ...
  if (af == AF_INET6 && ((__u32 *)addr)[0] == 0 && ((__u32 *)addr)[1] == 0 &&
      ((__u32 *)addr)[2] == htonl(0xffff)) {
    af = AF_INET;
    addr += 12;
    len = 4;
  }
  ...
  n->next = nht[hash];
  nht[hash] = n;
  if (++notfirst == 1) sethostent(1);
  fflush(stdout);

  if ((h_ent = gethostbyaddr(addr, len, af)) != NULL) n->name = strdup(h_ent->h_name);
...
const char *format_host(int af, int len, const void *addr, char *buf, int buflen) {
  ...
  if (resolve_hosts) {
    ...
    if (len > 0 && (n = resolve_address(addr, len, af)) != NULL) return n;
  }
#endif
...
__u8 *hexstring_a2n(const char *str, __u8 *buf, int blen) {
  ...
  if (cnt < blen) memset(buf + cnt, 0, blen - cnt);
  return buf;
}
...
size_t getcmdline(char **linep, size_t *lenp, FILE *in) {
  ...
  if ((cc = getline(linep, lenp, in)) < 0) return cc; /* eof or error */
  ++cmdlineno;
